...
def tinhtoan() :
    ...
    #Thực hiện tính toán khi đã người dùng đã nhập thông tin
    if  x.get()  != 0  and   cao2.get() != '' and nang2.get() != ''   :
        ...
        if x.get()== 1:
            ...
            print(bmi.get())
            if bmi.set(float(nang2.get())/(float(cao2.get())/100)**2) == a:
                thongbao1.configure( text = 'Mời bạn nhập đúng định dạng !! ', font= 10, fg = 'green')
            else:
...
tinh = Button(bg= 'blue',fg= 'white',text= 'CACULATE', font = ('arial', 16), command= tinhtoan )
...
thongbao1.place(x= 50,y=260)
thongbao2 = Label( text = ( ''), font= 16, fg = 'green')
thongbao2.place(x= 50,y=300)
BMI.mainloop()